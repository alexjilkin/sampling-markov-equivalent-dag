...
def count_equivalence_classes(steps):
    ...
    for G, _ in steps:
        ...
        G_cpdag_hash = get_graph_hash_ig(
            CPDAG_undirected) + get_graph_hash_ig(CPDAG_directed)
...
def sample(G: ig.Graph, size, is_markov_equivalent=False, is_REV=False):
    ...
    for i in range(int(size)):
        ...
        G_i_plus_1, step_type = propose_next(
            G_i, is_markov_equivalent, markov_prob, is_REV)
        ...
        if (step_type == 'REV'):
            print(
                f'{i} {current_score:.2f} {proposed_score:.2f} {AMOs} {get_es_diff(G_i_plus_1, G_i)}, {step_type}')
            G_i = G_i_plus_1
        elif (step_type):
...
def propose_protected_reverse(G_i: ig.Graph):
    ...
    G_i_plus_1: ig.Graph = G_i.copy()
    protected_edges = [
        e for e in G_i.es if is_strongly_protected(G_i, empty_G, e)]
